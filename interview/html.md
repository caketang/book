...
```
 ...
 但是b i 都是自然样式标签  分别表示无意义的加醋 无意义的斜体   在html4.01 不推荐使用

 而 em strong 都是语义样式标签  em 一般强调 strong 比em更强调    
 使用阅读设备时 strong会重读  b是强调展示内容

```
#### 32.前端需要注意哪些 SEO ？
```
1. 合理的 title、description、keywords
2. 语义话的html 代码  符合w3c规范 

3. 重要的HTML代码放在最前 
4. 重要内容不要用js输出  爬虫不回爬js
5. 少用iframe
6. 图片用atl属性
7. 提高网站速度 
```
...
####  37.iframe 有那些缺点？
```
iframe 会创建另一个文档的内联框架
主要缺点有：

1. ifram 阻塞onload事件 window 的 onload 事件需要在所有 iframe 加载完毕后（包含里面的元素）才
会触发。在 Safari 和 Chrome 里，通过 JavaScript 动态设置 iframe 的 src 可以避免这种阻塞情况。
2. 搜索引擎无法解读这种页面 无法做seo
...
自动完成允许浏览器预测输入  当用户在字段开始键入时，浏览器基于之前键入过的值，应该显示出在字段中填写的选项。
...
```
 ...
 第一种实现的方式是使用 websocket 协议，因为 websocket 协议可以实现服务器推送，所以`服务器`就可以用来当做这个中介者。
...
#### 49. 渐进增强和优雅降级的定义
```
渐进增强 针对低版本浏览器进行构建页面  保证最基本的功能  然后在针对高版本浏览器进行效果 和 交互改进和追加功能达到更好的体验
优雅降级 一开始就根据高版本浏览器构建完整的功能  在针对流低版本浏览器兼容
```

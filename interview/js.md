...
```
1.ES6提供了新的数据结构 set 类似于数组  成员唯一 没有重复的值
...
1. WeakMap结构与 Map 结构类似，也是用于生成键值对的集合。但是 WeakMap 只接受对象作为键名（ null 除外），不接受其他类型的值作为键名。而且 WeakMap 的键名所指向的对象，不计入垃圾回收机制。
```
...
#### 134. . Reflect 对象创建目的？
```
1. 将对象的一些明显属于内部的方法 放到Reflect上面
2. 修改某些Object方法的返回结果更合理
3. 让object操作变成函数行为
4. proxy 和 reflect 一一对应 只要是 Proxy 对象的方法，就能在 Reflect 对象上找到对应的方法。这就让 Proxy 对象可以方便地调用对应的 Reflect 方法，完成默认行为，作为修改行为的基础。也就是说，不管 Proxy 怎么修改默认行为，你总可以在 Reflect 上获取默认行为。
...
我主要在 MVP 模式解耦的时候用来将视图层的方法定义和方法调用分离。
...
#### 148.. Vue 组件间的参数传递方式？
```
1 父子通信 props  provide inject  refs
2 兄弟间  eventbus $parent.$refs
3 任意组件  eventbus 
vuex
```
